...
before looking at any solution. also want to implement the tabulation
(bottom-up) version of coin change as a follow-up, to compare it against the
memoised version and understand why it avoids recursion/stack depth issues.
...
to calculate the nth fibonacci number using memoisation, using 1-based indexing
(position 1 and 2 both return 1, position 3 returns 2, etc, matching how i'd
naturally count rather than starting at 0)
...
comparison-based relation like lcs had. i learnt the weight check ('items[i][0]
\<= j') has nothing to do with the memo table at all, it's just a real-world
feasibility check asking "does this item physically fit in the space i have
left". the memo table itself stores answers to subproblems, not weights or
values from the items list, i confused these two things initially. i also hit
...
than reading an explanation again. attempt a simpler 1d problem (climbing stairs
or house robber) next to consolidate rather than jumping to another hard 2d
...
intuition alone isn't a reliable enough check. attempt house robber next, which
introduces an actual choice at each step (rob this house or don't) rather than a
fixed relation, closer in spirit to knapsack's take/leave logic but in 1d. --- - ##
house robber ### goal given an array of house values along a street, choose a
subset of houses to rob that maximises total value, without robbing two adjacent
houses. ### what i learnt this introduced a genuine choice per element, similar
in spirit to knapsack's take/leave, but in 1d. the two options are rob the
current house (add its value, then skip to two houses back) or don't rob it
(move to just one house back), and the answer at each step is the max of those
two paths. answering the three questions before coding this time (what does the
parameter represent, what happens under each choice, what's the base case)
genuinely helped, compared to climbing stairs where i skipped that step and it
happened to work out anyway. writing out that robbing moves the index back by 2
and not robbing moves it back by 1 made the recursive calls obvious before i
wrote any code. ### challenges my first working attempt had two real bugs: i
recursed on the "rob" path without ever adding the actual value of the house
being robbed, and my base case (`num_houses >= 0`) let the recursion continue
into negative territory, which would have caused an out-of-bounds memo access
and crashed. the second bug is a repeat of a pattern i've now hit a few times,
checking the memo or doing work before confirming the base case properly stops
the recursion. i'm starting to recognise this as a habit to specifically watch
for: base case first, fully stopping any further recursion or indexing, before
anything else happens in the function. ### changes for next time specifically
double check, for every new dp problem, that the base case is checked first and
correctly prevents any negative or out-of-bounds indexing before the memo check
or recursive calls happen. consider attempting house robber ii (houses in a
circle, first and last are also adjacent) to test adapting an already-solved
pattern to a new constraint, rather than starting a completely new problem from
scratch each time.
